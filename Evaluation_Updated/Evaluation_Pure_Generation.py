...
def ergas(pred: torch.Tensor, tgt: torch.Tensor, mask: Optional[torch.Tensor], scale_ratio: float = 4.0) -> float:
    """ERGAS metric (lower is better). scale_ratio is HR/LR (e.g., 10m/40m -> use 4.0)."""
    C = pred.size(1)
    rmse_sq = 0.0
    for c in range(C):
...
# ---------------------- baselines ----------------------
@torch.no_grad()
def baseline_bicubic(x_gt: torch.Tensor, scale_factor: int = 4) -> torch.Tensor:
    """SISR bicubic baseline on GT itself: down x4 then up x4 back, as a sanity SR baseline."""
    C,H,W = x_gt.size(1), x_gt.size(2), x_gt.size(3)
    lr = F.interpolate(x_gt, scale_factor=1.0/scale_factor, mode='bicubic', align_corners=False, recompute_scale_factor=True)
    sr = F.interpolate(lr, size=(H,W), mode='bicubic', align_corners=False)
    return torch.clamp(sr, 0.0, 1.0)

@torch.no_grad()
def baseline_linear_from_s1(x_cond: torch.Tensor, Ct: int, mask: Optional[torch.Tensor], subsample_max: int = 200_000, ridge: float = 1e-4) -> torch.Tensor:
    """Linear regression per S2 band: upsampled S1 channels -> S2 bands (per-pixel linear + bias).
       Returns prediction tensor with shape like (1,Ct,H,W).
    """
    dev = x_cond.device
    B,Cc,H,W = x_cond.shape
    assert B == 1
    # Prepare mask of valid pixels
    if mask is not None:
        m = (mask.squeeze(0) > 0).flatten().detach().cpu().numpy().astype(bool)
    else:
        m = np.ones(H*W, dtype=bool)
    # Flatten S1 condition
    X = x_cond.squeeze(0).detach().cpu().numpy().reshape(Cc, -1).T  # (N, Cc)
    X = X[m]
    # Add bias term
    Xb = np.concatenate([X, np.ones((X.shape[0],1), dtype=np.float32)], axis=1)  # (N, Cc+1)
    # Optionally subsample for speed
    N = Xb.shape[0]
    if N > subsample_max:
        idx = np.random.choice(N, subsample_max, replace=False)
        Xb_sub = Xb[idx]
    else:
        idx = None
        Xb_sub = Xb
    # Precompute normal matrix with ridge
    XtX = Xb_sub.T @ Xb_sub
    d = XtX.shape[0]
    XtX += ridge * np.eye(d, dtype=np.float32)
    XtX_inv = np.linalg.inv(XtX)
    # Prepare output container
    pred = torch.zeros((1, Ct, H, W), device=dev, dtype=torch.float32)
    # Fit each band independently
    # We'll require GT externally to compute metrics; here we just need to map S1->S2 prediction
    # For fitting we need y for each band; to keep function generic, caller provides GT and we compute here
    return pred  # caller will fill using helper below

@torch.no_grad()
def baseline_linear_fit_predict(x_cond: torch.Tensor, x_gt: torch.Tensor, mask: Optional[torch.Tensor], subsample_max: int = 200_000, ridge: float = 1e-4) -> torch.Tensor:
    """Fit per-band linear map from S1 (x_cond) to S2 (x_gt) on the SAME image (diagnostic baseline),
       then predict S2. This is an optimistic baseline but useful for reference.
    """
    dev = x_cond.device
    B,Cc,H,W = x_cond.shape
    Ct = x_gt.size(1)
    ...
    XtX_inv = np.linalg.inv(XtX)
    # Predict per band
    pred = torch.zeros_like(x_gt)
    for c in range(Ct):
        y = x_gt[:,c:c+1].squeeze(0).detach().cpu().numpy().reshape(-1)[m]
        y_sub = y_prepare(y)
        Xty = Xb_sub.T @ y_sub
        w = XtX_inv @ Xty  # (Cc+1,)
        # full prediction
        yhat = (Xb @ w).astype(np.float32)
        # place back
        full = np.zeros(H*W, dtype=np.float32)
        full[m] = yhat
        pred[:,c:c+1] = torch.from_numpy(full.reshape(1,1,H,W)).to(dev)
...
# ---------------------- main (batch) ----------------------
def main():
    ap = argparse.ArgumentParser("Batch eval & viz suite (with true-infer switch and baselines)")
    ap.add_argument("--mode", required=True, choices=["tsweep","ddim","eps","seed_stats","per_band","ablate","cloudy_viz","night_demo","baseline_bicubic","baseline_linear"]) 
    ap.add_argument("--patch_dir", required=True)
    ap.add_argument("--ckpt", required=False, help="Required for model-based modes")
    ...
    ap.add_argument("--true_infer", action="store_true", help="Use true inference scoring (start from noise, only S1 condition)")
    ...
    # Warm up first file to build the model when needed
    need_model = args.mode in ["tsweep","ddim","eps","seed_stats","per_band","ablate","cloudy_viz","night_demo"]
    if need_model:
        assert args.ckpt is not None and os.path.exists(args.ckpt), "--ckpt is required for model-based modes"
    ...
    # ---------------- MODE: DDIM (recon vs true inference) ----------------
    elif args.mode == "ddim":
        if args.true_infer:
            csv_path = os.path.join(args.out_dir, "ddim_true_infer_metrics.csv")
            maes_mu, mses_mu, psnrs_mu, sams_mu, ergases_mu = [], [], [], [], []
            with open(csv_path, "w", newline="") as f:
                hdr = ["file","t_start","ddim_steps","seeds","MAE_mean","MAE_std","MSE_mean","MSE_std","PSNR_mean","SAM_mean","ERGAS_mean"]
                w = csv.writer(f); w.writerow(hdr)
                for i, fname in enumerate(files):
                    x_cond, x_gt, mask, Cc, Ct, _ = load_npz_as_tensors(os.path.join(args.patch_dir, fname), device)
                    per_mae, per_mse, per_psnr, per_sam, per_ergas = [], [], [], [], []
                    for s in range(args.n_seeds):
                        ...
                        mae = masked_mae(x0, x_gt, mask); mse = masked_mse(x0, x_gt, mask)
                        psn = psnr(x0, x_gt, mask); sa = sam(x0, x_gt, mask); eg = ergas(x0, x_gt, mask)
                        per_mae.append(mae); per_mse.append(mse); per_psnr.append(psn); per_sam.append(sa); per_ergas.append(eg)
                    mu = lambda a: float(np.mean(a)); sd = lambda a: float(np.std(a, ddof=0))
                    w.writerow([fname, args.t_start, args.ddim_steps, args.n_seeds,
                                f"{mu(per_mae):.6f}", f"{sd(per_mae):.6f}",
                                f"{mu(per_mse):.6f}", f"{sd(per_mse):.6f}",
                                f"{mu(per_psnr):.3f}", f"{mu(per_sam):.4f}", f"{mu(per_ergas):.2f}"])
                    if i < args.save_viz_n:
                        save_panel(x0, x_gt, x_cond, mask,
                                   os.path.join(viz_dir, f"{i:03d}_gen_panel.png"),
                                   title=f"True-infer DDIM t_start={args.t_start}, steps={args.ddim_steps}",
                                   zoom=args.zoom, zoom_k=args.zoom_k)
                    maes_mu.append(mu(per_mae)); mses_mu.append(mu(per_mse)); psnrs_mu.append(mu(per_psnr)); sams_mu.append(mu(per_sam)); ergases_mu.append(mu(per_ergas))
            def mstd(a): t = torch.tensor(a); return t.mean().item(), t.std(unbiased=False).item()
            with open(os.path.join(args.out_dir, "ddim_true_infer_summary.txt"), "w") as f:
                f.write(f"files: {len(files)}  t_start: {args.t_start}  steps: {args.ddim_steps}  seeds: {args.n_seeds}\n")
                f.write(f"MAE mean/std:  {mstd(maes_mu)[0]:.6f} / {mstd(maes_mu)[1]:.6f}\n")
                f.write(f"MSE mean/std:  {mstd(mses_mu)[0]:.6f} / {mstd(mses_mu)[1]:.6f}\n")
                f.write(f"PSNR mean/std: {mstd(psnrs_mu)[0]:.3f} / {mstd(psnrs_mu)[1]:.3f}\n")
                f.write(f"SAM  mean/std: {mstd(sams_mu)[0]:.4f} / {mstd(sams_mu)[1]:.4f}\n")
                f.write(f"ERGAS mean/std:{mstd(ergases_mu)[0]:.2f} / {mstd(ergases_mu)[1]:.2f}\n")
            print("[DONE] DDIM (TRUE-INFER)")
        else:
    ...
    # ---------------- MODE: CLOUDY_VIZ ----------------
        ...
        for i, fname in enumerate(selected):
            x_cond, x_gt, mask, Cc, Ct, _ = load_npz_as_tensors(os.path.join(args.patch_dir, fname), device)
            _, _, x0 = ddim_multistep_eval(model, x_gt, x_cond, alpha_bar, mask, t_start=args.t_start, steps=args.ddim_steps)
            save_panel(x0, x_gt, x_cond, mask,
    ...
    # ---------------- MODE: BASELINE_BICUBIC ----------------
    elif args.mode == "baseline_bicubic":
        csv_path = os.path.join(args.out_dir, "baseline_bicubic.csv")
        maes, mses, psnrs, sams, ergases = [], [], [], [], []
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f); w.writerow(["file","MAE","MSE","PSNR","SAM(rad)","ERGAS"]) 
            for i, fname in enumerate(files):
                _, x_gt, mask, _, _, _ = load_npz_as_tensors(os.path.join(args.patch_dir, fname), device)
                x0 = baseline_bicubic(x_gt, scale_factor=4)
                mae = masked_mae(x0, x_gt, mask); mse = masked_mse(x0, x_gt, mask)
                psn = psnr(x0, x_gt, mask); sa = sam(x0, x_gt, mask); eg = ergas(x0, x_gt, mask)
                maes.append(mae); mses.append(mse); psnrs.append(psn); sams.append(sa); ergases.append(eg)
                w.writerow([fname, f"{mae:.6f}", f"{mse:.6f}", f"{psn:.3f}", f"{sa:.4f}", f"{eg:.2f}"])
        def mstd(a): t = torch.tensor(a); return t.mean().item(), t.std(unbiased=False).item()
        with open(os.path.join(args.out_dir, "baseline_bicubic_summary.txt"), "w") as f:
            ...
            f.write(f"SAM  mean/std: {mstd(sams)[0]:.4f} / {mstd(sams)[1]:.4f}\n")
            f.write(f"ERGAS mean/std:{mstd(ergases)[0]:.2f} / {mstd(ergases)[1]:.2f}\n")
        print("[DONE] BASELINE_BICUBIC")

    # ---------------- MODE: BASELINE_LINEAR (S1->S2 per-band linear fit) ----------------
    elif args.mode == "baseline_linear":
        csv_path = os.path.join(args.out_dir, "baseline_linear.csv")
        maes, mses, psnrs, sams, ergases = [], [], [], [], []
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f); w.writerow(["file","MAE","MSE","PSNR","SAM(rad)","ERGAS"]) 
            for i, fname in enumerate(files):
                x_cond, x_gt, mask, Cc, Ct, _ = load_npz_as_tensors(os.path.join(args.patch_dir, fname), device)
                x0 = baseline_linear_fit_predict(x_cond, x_gt, mask, subsample_max=200_000, ridge=1e-4)
                mae = masked_mae(x0, x_gt, mask); mse = masked_mse(x0, x_gt, mask)
                psn = psnr(x0, x_gt, mask); sa = sam(x0, x_gt, mask); eg = ergas(x0, x_gt, mask)
                maes.append(mae); mses.append(mse); psnrs.append(psn); sams.append(sa); ergases.append(eg)
                w.writerow([fname, f"{mae:.6f}", f"{mse:.6f}", f"{psn:.3f}", f"{sa:.4f}", f"{eg:.2f}"])
        def mstd(a): t = torch.tensor(a); return t.mean().item(), t.std(unbiased=False).item()
        with open(os.path.join(args.out_dir, "baseline_linear_summary.txt"), "w") as f:
            f.write(f"files: {len(files)}\n")
            f.write(f"MAE mean/std:  {mstd(maes)[0]:.6f} / {mstd(maes)[1]:.6f}\n")
            f.write(f"MSE mean/std:  {mstd(mses)[0]:.6f} / {mstd(mses)[1]:.6f}\n")
            f.write(f"PSNR mean/std: {mstd(psnrs)[0]:.3f} / {mstd(psnrs)[1]:.3f}\n")
