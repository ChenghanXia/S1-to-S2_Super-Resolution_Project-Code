...
def main():
    ...
    ap.add_argument("--mode", required=True, choices=["ddim","eps","seed_stats","per_band","ablate","ddim_sweep"])
    ...
    # DDIM sweep (grid search)
    ap.add_argument("--t_start_grid", type=str, default="", help="comma-separated t_start list, e.g. '300,200,150,100'")
    ap.add_argument("--ddim_steps_grid", type=str, default="", help="comma-separated steps list, e.g. '10,20,50,100'")
    ...
    if args.mode == "ddim":
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for i, fname in enumerate(files):
                ...
                # Fix per-file initial noise for fair comparison across configs
    ...
    # MODE: DDIM_SWEEP (grid) 
    elif args.mode == "ddim_sweep":
        assert args.t_start_grid and args.ddim_steps_grid, "Provide --t_start_grid and --ddim_steps_grid"
        t_list = [int(x) for x in args.t_start_grid.split(",")]
        k_list = [int(x) for x in args.ddim_steps_grid.split(",")]

        sweep_csv = os.path.join(args.out_dir, "ddim_sweep_summary.csv")
        ensure_dir(args.out_dir)
        with open(sweep_csv, "w", newline="") as fsum:
            wsum = csv.writer(fsum)
            wsum.writerow(["t_start","steps","files","MAE_mean","MAE_std","MSE_mean","MSE_std"])
            for t_start in t_list:
                for steps in k_list:
                    ...
                    for i, fname in enumerate(files):
                        x_cond, x_gt, mask, Cc, Ct = load_npz_as_tensors(os.path.join(args.patch_dir, fname), device)
                        assert Cc==Cc0 and Ct==Ct0, "Channel mismatch across files"
                        # Fix per-file initial noise: same file uses same starting noise across all configs
                        torch.manual_seed(args.seed_base + i)
                        mae, mse, _ = ddim_multistep_eval(
                            model, x_gt, x_cond, alpha_bar, mask,
                            t_start=t_start, steps=steps
                        )
                        maes.append(mae); mses.append(mse)
                    maes_t = torch.tensor(maes); mses_t = torch.tensor(mses)
                    wsum.writerow([
                        t_start, steps, len(files),
                        f"{maes_t.mean().item():.6f}", f"{maes_t.std(unbiased=False).item():.6f}",
                        f"{mses_t.mean().item():.6f}", f"{mses_t.std(unbiased=False).item():.6f}",
                    ])
        print(f"[DONE] DDIM sweep -> {sweep_csv}")
